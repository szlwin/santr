...
class ToolUtil {
	...
	protected boolean isLastTree(GTree lbs) {
		...
		int index = parentTree
				.getgTreeArray()
				.length-1;
		return  (lbs.getGroup() == 0 || lbs.getGroupType() == GROUPTYPE.ZERO_OR_ONE)
	...
	protected RTree createNodeTree(RTree rTree,TokenString tokenString) throws ParserInvaildException 
	{
		...
		if(lbs == null){
			throw new ParserInvaildException(getTokenString().getText());
		}
	...
	protected void createAllPreTree(GTree cleaf, RTree currentTree) throws ParserInvaildException{
		...
		int count = 0;
		GTree[] gTreeArr = cleaf.getgTreeArray();
		while(unLindex < tokenStream.getCurrentIndex()-1){
			...
			if(tmpToken.getType() == TOKENTYPE.TOKEN){
				...
				rTree.setLbs(gTreeArr[count]);
				currentTree.addRTree(rTree);
			}else{
				GTree lbs = gTreeArr[count];
