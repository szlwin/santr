...
public class PathScaner {
	...
	private void scanBother(GTree gTree){
		GTree gTreeArr[] = gTree.getParent().getgTreeArray();
		for(GTree childTree:gTreeArr){
			if(childTree.getToken()!=null 
					&& childTree.getToken().containsKey(tId)){
				LineInfo lineInfo = createBotherLine(childTree);
	...
	private void convert(){
		for(int i = 0;i < runnerList.size();i++){
			...
			LineInfo lineInfo = createDownLine(gTree,runner,1);
			lineInfoList.add(lineInfo);
		}
	}
	...
	private LineInfo createBotherLine(GTree gTree){
	...
	private LineInfo createDownLine(GTree gTree,Runner runner,int deep){
		LineInfo lineInfo = new LineInfo();
		lineInfo.setStart(first.getId());
		
		lineInfo.setEnd(gTree.getId());
		
		GTree runFirst = runner.getFirst();
		
		if(runFirst.getType() == GTYPE.LEAF){
			//lineInfo.setEnd(gTree.getParent().getId());
			lineInfo.setLeafId(runFirst.getId());
		}else{
			//lineInfo.setEnd(gTree.getId());
			lineInfo.setLeafId(-1);
		}
